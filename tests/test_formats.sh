...
uv run brcschema transform -T osti_to_brc -o /tmp/demo.json "$FIXTURES_DIR/shewanella.json" > /dev/null 2>&1
uv run python <<'EOF'
import json
...
uv run brcschema transform -T osti_to_brc -o /tmp/demo.yaml "$FIXTURES_DIR/shewanella.json" > /dev/null 2>&1
uv run python <<'EOF'
import yaml
...
uv run brcschema transform -T osti_to_brc -o /tmp/demo.yml "$FIXTURES_DIR/shewanella.json" > /dev/null 2>&1
uv run python <<'EOF'
import yaml
