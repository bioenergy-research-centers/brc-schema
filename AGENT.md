...
- Species mapping (`build_brc_species` in `transform.py`) uses `src/brc_schema/transform/organisms.yaml` plus live NCBI Taxonomy lookups via oaklib/OLS (`src/brc_schema/util/taxonomy.py`). The `feed_organisms` block at the end of `organisms.yaml` is generated by `scripts/update_organisms_from_feeds.py`; do not hand-edit it, put corrections in the curated `organisms` / `ambiguous_names` sections and rerun. Tests must stay offline: `tests/conftest.py` routes every lookup to a fake adapter, so add taxa there when a test needs them.
