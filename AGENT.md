...
- Species mapping (`build_brc_species` in `transform.py`) uses `src/brc_schema/transform/organisms.yaml` plus live NCBI Taxonomy lookups via oaklib/OLS (`src/brc_schema/util/taxonomy.py`). Tests must stay offline: `tests/conftest.py` routes every lookup to a fake adapter, so add taxa there when a test needs them.
